...
class GVLService {
  ...
  Future<bool> _loadFromCache() async {
    try {
      ...
      print('GVLService: Loaded ${_cachedVendors?.length ?? 0} vendors from cache (v$_cachedVersion)');
      return true;
    } catch (error) {
  ...
  Future<bool> _fetchFromRemote(String? specificVersion) async {
    try {
      ...
      if (response.statusCode != 200) {
        print('GVLService: HTTP ${response.statusCode} - ${response.reasonPhrase}');
        return false;
      }
      ...
      print('GVLService: Loaded ${_cachedVendors?.length ?? 0} vendors from remote (v$_cachedVersion)');
      return true;
    } catch (error) {
    ...
    try {
      ...
      for (final entry in vendorsData.entries) {
        ...
        vendors[vendorId] = VendorInfo(
          ...
          usesNonCookieAccess: vendorData['usesNonCookieAccess'] as bool? ?? false,
  ...
  Future<void> _saveToCache(String rawData, String version) async {
    try {
      ...
      await prefs.setInt(_cacheKeyGvlTimestamp, DateTime.now().millisecondsSinceEpoch);
    } catch (error) {
