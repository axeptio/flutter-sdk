...
void main() {
  ...
  group('Normal Operations', () {
    ...
    group('Data Retrieval', () {
      ...
      test('getConsentSavedData returns null for non-existent key', () async {
        final result = await platform.getConsentSavedData(
            preferenceKey: 'non_existent_key');
  ...
  group('Error Handling', () {
    ...
    setUp(() {
      TestDefaultBinaryMessengerBinding.instance.defaultBinaryMessenger
          .setMockMethodCallHandler(const MethodChannel('axeptio_sdk'),
              (call) async {
        // Mock platform exceptions for error testing
        switch (call.method) {
          case 'getPlatformVersion':
            throw PlatformException(
                code: 'UNAVAILABLE', message: 'Platform version unavailable');
          case 'getVendorConsents':
            throw PlatformException(
                code: 'FETCH_ERROR',
                message: 'Failed to fetch vendor consents');
          case 'getConsentSavedData':
          ...
          case 'getVendorName':
            ...
            if (vendorId < 0) {
              throw PlatformException(
                  code: 'INVALID_ID', message: 'Invalid vendor ID');
            }
    ...
    test('data parsing handles invalid formats', () async {
      ...
      expect(
          consentedVendors, contains(1)); // Should filter out invalid entries
