...
class AxeptioSdkPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    override fun onMethodCall(call: MethodCall, result: Result) {
        when (call.method) {
            ...
            "loadGVL" -> {
                val gvlVersion = call.argument<String>("gvlVersion")
                AxeptioGVLManager.loadGVL(gvlVersion) { success ->
                    result.success(success)
                }
            }
            "unloadGVL" -> {
                AxeptioGVLManager.unloadGVL()
                result.success(null)
            }
            "clearGVL" -> {
                AxeptioGVLManager.clearGVL()
                result.success(null)
            }
            ...
            "getVendorName" -> {
                val vendorId = call.argument<Int>("vendorId")
                if (vendorId != null) {
                    val vendorName = AxeptioGVLManager.getVendorName(vendorId)
                    result.success(vendorName)
                } else {
                    result.error("invalid_args", "getVendorName: Missing argument 'vendorId'", null)
                }
            }
            "getVendorNames" -> {
                val vendorIds = call.argument<List<Int>>("vendorIds")
                if (vendorIds != null) {
                    val vendorNames = AxeptioGVLManager.getVendorNames(vendorIds)
                    val safeResponse = sanitizeForFlutter(vendorNames)
                    result.success(safeResponse)
                } else {
                    result.error("invalid_args", "getVendorNames: Missing argument 'vendorIds'", null)
                }
            }
            "getVendorConsentsWithNames" -> {
                val vendorConsents = AxeptioSDK.instance().getVendorConsents()
                val vendorInfos = mutableMapOf<String, Map<String, Any>>()
                
                vendorConsents.forEach { (vendorId, consented) ->
                    val vendorInfo = AxeptioGVLManager.getVendorInfo(vendorId)
                    if (vendorInfo != null) {
                        val info = vendorInfo.toMutableMap()
                        info["consented"] = consented
                        vendorInfos[vendorId.toString()] = info
                    } else {
                        // Fallback for vendors not in GVL
                        vendorInfos[vendorId.toString()] = mapOf(
                            "id" to vendorId,
                            "name" to "Vendor $vendorId",
                            "consented" to consented,
                            "purposes" to emptyList<Int>(),
                            "legIntPurposes" to emptyList<Int>(),
                            "specialFeatures" to emptyList<Int>(),
                            "specialPurposes" to emptyList<Int>(),
                ...
                val safeResponse = sanitizeForFlutter(vendorInfos)
                result.success(safeResponse)
            }

            // GVL status methods
            "isGVLLoaded" -> {
                val isLoaded = AxeptioGVLManager.isGVLLoaded()
                result.success(isLoaded)
            }
            "getGVLVersion" -> {
                val version = AxeptioGVLManager.getGVLVersion()
